...
ok * dass man die selektionparameter an/abhaken kann (ich brauche zb fast ausschließlich FWHM arcsec (der wäre super) und die Eccentricity) und diese als Profil speichern kann
...
ok .) Bewertungsfilter an/abhaken (alle sollten frei wählbar sein)
ok .) automatisch dieses laden lassen können (autocalculation oder profil sollte wählbar sein)
ok .) wenn gewisse Parameter abgewählt sind diese gar nicht anzeigen lassen
...
Ein Fehler gefunden: keine „stars“ eingestellt, aber er wirft mir Bilder deswegen raus

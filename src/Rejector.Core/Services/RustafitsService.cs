...
{
    public sealed record RenderedImage(int Width, int Height, byte[] Rgb24Data, int Stride);
    ...
    public async Task<ProcessedFrame> ProcessFrameAsync(string filePath, CancellationToken cancellationToken)
    {
    ...
    public Task<PreviewRenderResult> RenderPreviewImagesAsync(LoadedFrame frame, StfParameters stf, (double Left, double Top, double Width, double Height)? roiNormalizedRect, AstroMetrics? metrics, CancellationToken cancellationToken)
    {
        return Task.Run(() =>
        {
            if (frame.IsOsc && frame.ColorChannels is { Length: 3 } cc)
            {
                ...
                return new PreviewRenderResult(full, roi);
            }
            ...
            return new PreviewRenderResult(monoFull, monoRoi);
        }, cancellationToken);
    }
    ...
    public Task<RenderedImage> RenderScaledPreviewImageAsync(LoadedFrame frame, int targetWidth, int targetHeight, StfParameters stf, CancellationToken cancellationToken)
    {
        return Task.Run(() =>
        {
            if (frame.IsOsc && frame.ColorChannels is { Length: 3 } cc)
    ...
    private static bool TryReadNamedMetadataCollection(object source, string collectionPropertyName, string key, out double value)
    {
        ...
        if (tryGetPropertyMethod is not null)
        {
            ...
            var metadata = args[1];
            if (tryGetPropertyMethod.Invoke(collection, args) is true && metadata is not null && TryConvertMetadataValue(metadata, out value))
            {
                return true;
            }
        }
    ...
    private static RenderedImage CreateThumbnailBitmapColor(
        float[] rCh, float[] gCh, float[] bCh,
        ...
        AstroMetrics? metrics, double normalizationMax)
    {
        ...
        return new RenderedImage(maxWidth, maxHeight, sample, maxWidth * 3);
    }

    private static RenderedImage CreateRoiBitmapColor(
        float[] rCh, float[] gCh, float[] bCh,
        ...
        double normalizationMax)
    {
        ...
        return new RenderedImage(roiSize, roiSize, sample, roiSize * 3);
    }

    private static RenderedImage CreateFullFrameBitmapColor(
        float[] rCh, float[] gCh, float[] bCh,
        ...
        double normalizationMax)
    {
        ...
        return new RenderedImage(width, height, sample, width * 3);
    }

    private static RenderedImage CreateScaledFrameBitmapColor(
        float[] rCh, float[] gCh, float[] bCh,
    ...
    private static RenderedImage CreateThumbnailBitmap(float[] pixels, int width, int height, int maxWidth, int maxHeight, StfParameters stf, AstroMetrics? metrics, double normalizationMax)
    {
        var scale = Math.Min(maxWidth / (double)Math.Max(1, width), maxHeight / (double)Math.Max(1, height));
        scale = Math.Min(1.0, scale <= 0 ? 1.0 : scale);
        var contentWidth = Math.Max(1, (int)Math.Round(width * scale));
        ...
        return new RenderedImage(maxWidth, maxHeight, sample, stride);
    }
    ...
    private static RenderedImage CreateRoiBitmap(float[] pixels, int width, int height, int roiSize, StfParameters stf, (double Left, double Top, double Width, double Height)? roiNormalizedRect, double normalizationMax)
    {
        int startX, startY, cropW, cropH;
        if (roiNormalizedRect is { } rect)
        {
        ...
        return new RenderedImage(roiSize, roiSize, sample, stride);
    }

    private static RenderedImage CreateFullFrameBitmap(float[] pixels, int width, int height, StfParameters stf, double normalizationMax)
    {
        var sample = DownsampleAndStretch(pixels, width, height, width, height, stf, normalizationMax);
        var stride = width * 3;
        return new RenderedImage(width, height, sample, stride);
    }

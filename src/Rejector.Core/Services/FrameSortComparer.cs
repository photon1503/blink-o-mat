...
public sealed class FrameSortComparer : IComparer<ProcessedFrame>
{
    ...
    private static int CompareByRule(ProcessedFrame left, ProcessedFrame right, FrameSortRule rule)
    {
        return rule.Field switch
        {
            ...
            _ when IsField(rule.Field, "cloud confidence") => CompareValues(left.CloudConfidence, right.CloudConfidence, rule.IsAscending),
            ...
            _ when IsField(rule.Field, "minimum pixel value", "min") => CompareValues(left.Metrics.Min, right.Metrics.Min, rule.IsAscending),
            _ when IsField(rule.Field, "minimum pixel count", "min count") => CompareValues(left.Metrics.MinCount, right.Metrics.MinCount, rule.IsAscending),
            _ when IsField(rule.Field, "maximum pixel value", "max") => CompareValues(left.Metrics.Max, right.Metrics.Max, rule.IsAscending),
            _ when IsField(rule.Field, "maximum pixel count", "max count") => CompareValues(left.Metrics.MaxCount, right.Metrics.MaxCount, rule.IsAscending),
    ...
    private static bool IsField(string? fieldName, params string[] expectedNames)
    {
        var normalizedField = NormalizeField(fieldName);
        return expectedNames.Any(expected => string.Equals(normalizedField, NormalizeField(expected), StringComparison.OrdinalIgnoreCase));
    }
